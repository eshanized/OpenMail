...
>
        ...
        <div class="p-4 space-y-3">
            @foreach(range(1, 5) as $i)
                <div class="flex items-center gap-3 animate-pulse">
                    <div class="w-3 h-3 rounded-full bg-surface-sunken"></div>
                    <div class="h-4 bg-surface-sunken rounded flex-1"></div>
                    <div class="h-4 bg-surface-sunken rounded w-8"></div>
                </div>
            @endforeach
        </div>
    @elseif($labels->isEmpty())
        ...
        <div class="flex-1 flex flex-col items-center justify-center p-6 text-center">
                            <svg class="w-12 h-12 text-ink-tertiary mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
            <p class="text-sm font-medium text-ink mb-1">No labels created</p>
            <p class="text-xs text-ink-tertiary mb-3">Create labels to organize messages across folders.</p>
            <button
                wire:click="openCreateModal"
                ...
                >
                    ...
                    <div class="flex items-center gap-2 ml-2">
                        @if($label->unread_count > 0)
                            <span class="text-xs font-bold text-primary bg-primary-subtle px-1.5 py-0.5 rounded-full">
                                {{ $label->unread_count }}
                            </span>
                        @endif
                        <span class="text-xs text-ink-tertiary">{{ $label->total_count }}</span>
                    </div>
                    ...
                    <div class="hidden group-hover:flex items-center gap-1 ml-2">
                        <button
                            ...
                            class="p-1 text-ink-tertiary hover:text-ink-secondary rounded"
                            title="Edit"
                        >
                        ...
                        <button
                            ...
                            class="p-1 text-ink-tertiary hover:text-danger rounded"
                            title="Delete"
                        >
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div
        ...
        x-cloak
        class="fixed z-50 bg-white rounded-lg shadow-lg border border-border py-1 min-w-[160px]"
    ...
    >
        <button
            ...
            class="w-full px-4 py-2 text-left text-sm text-ink-secondary hover:bg-surface-sunken flex items-center gap-2"
        >
        ...
        <button
            ...
            class="w-full px-4 py-2 text-left text-sm text-danger hover:bg-danger-subtle/50 flex items-center gap-2"
        >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
