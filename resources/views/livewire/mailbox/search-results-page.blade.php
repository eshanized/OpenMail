        ...
        <aside class="w-72 flex-shrink-0 hidden lg:block">
            <div class="bg-white rounded-lg border border-gray-200 p-4 sticky top-6">
                ...
                <form method="GET" action="{{ route('search') }}" id="filter-form">
                    ...
                    {{-- Folder Filter --}}
                    <div class="mb-4">
                        <label for="filter-folder" class="block text-xs font-medium text-gray-600 mb-1">Folder</label>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex flex-wrap gap-1">
                            ...
                            @foreach($datePresets as $preset => $label)
                                <a href="{{ route('search', array_merge(request()->query(), ['date_preset' => $preset, 'date_from' => null, 'date_to' => null])) }}"
                                   wire:navigate
                                   class="px-2 py-1 text-xs rounded {{ $activePreset === $preset ? 'bg-blue-100 text-blue-700 font-medium' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                                    {{ $label }}
                                </a>
                            @endforeach
                        </div>
                        <div class="mt-2 grid grid-cols-2 gap-2">
                            <div>
                                <label for="date_from" class="block text-xs text-gray-500">From</label>
                            ...
                            <div>
                                <label for="date_to" class="block text-xs text-gray-500">To</label>
                    ...
                    <div class="mb-4">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Labels</label>
                        <div class="space-y-1 max-h-40 overflow-y-auto">
                            @foreach($labels as $label)
                                <label class="flex items-center gap-2 cursor-pointer">
                                    ...
                                    <span class="text-sm text-gray-700 truncate">{{ $label->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @foreach($activeFilters as $filter)
                        ...
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                            ...
                            <a href="{{ route('search', $remainingParams) }}" wire:navigate
                               class="ml-0.5 text-gray-500 hover:text-gray-700" aria-label="Remove filter">
                            ...
                            <a href="{{ route('message.show', ['folderPath' => $result->folder_path, 'uid' => $result->uid]) }}"
                               ...
                               class="block px-4 py-3 hover:bg-gray-50 transition-colors">
                                <div class="flex items-start gap-3">
                                    <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-medium text-sm flex-shrink-0">
                                        {{ mb_substr($result->from_name ?? $result->from_address ?? '?', 0, 1) }}
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium text-gray-900 truncate">{{ $result->from_name ?? $result->from_address }}</span>
                                            <span class="text-xs text-gray-400 flex-shrink-0">{{ $result->formatted_date }}</span>
