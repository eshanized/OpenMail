<div class="p-8 sm:p-12">
    ...
    @if (!$verificationRun)
        <div class="setup-panel text-center py-10 px-6 border-dashed">
            <div class="w-10 h-10 rounded bg-primary/10 text-primary flex items-center justify-center mx-auto mb-4">
    ...
    @if ($verificationRun)
            ...
            @foreach ($verificationResults as $check)
                <div class="setup-panel !p-4 flex items-start justify-between gap-4 transition-all">
                    <div class="flex-1">
                        ...
                        @if (!$check['passed'] && isset($check['fixStep']) && $check['fixStep'])
                            <button type="button"
                                    ...
                                    class="mt-2 text-xs font-medium text-primary hover:text-primary-hover hover:underline focus:outline-none flex items-center gap-1">
                        ...
                        @if (isset($check['technical']) && $check['technical'])
                            <div x-data="{ open: false }" class="mt-2">
                                ...
                                <pre x-show="open"
                                     x-transition
                                     class="mt-1.5 p-2.5 bg-black/40 rounded text-xs font-mono text-ink-secondary overflow-auto max-h-32 border border-border">{{ json_encode($check['technical'], JSON_PRETTY_PRINT) }}</pre>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex-shrink-0">
                        @if ($check['passed'])
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-xs font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                Pass
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-xs font-medium bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20">
        ...
        <div class="mt-4 flex justify-end">
            <button type="button"
                    wire:click="runVerification"
                    wire:loading.attr="disabled"
                    class="text-xs font-medium text-primary hover:text-primary-hover hover:underline inline-flex items-center gap-1">
        ...
        @if ($allPassed)
            <div class="mt-8 flex justify-end">
                <button type="button"
                        ...
                        wire:loading.attr="disabled"
                        wire:target="finish"
                        class="setup-btn-primary !px-6 !py-2.5 text-sm">
