...
>
    @php
        $displaySender = $thread->from_display ?? 'Unknown';
        $avatarChar = strtoupper(substr($displaySender, 0, 1));
        $avatarIdx = abs(crc32($displaySender)) % 6;
    @endphp
    <div class="flex items-center px-4 py-3 hover:bg-hover transition-colors duration-75 group">
        {{-- Checkbox --}}
        <input
            type="checkbox"
            @if(in_array($thread->uid, $selectedUids ?? [])) checked @endif
            @click.stop="$dispatch('toggle-uid', { uid: {{ $thread->uid }} })"
            class="w-4 h-4 text-primary border-border-strong rounded focus:ring-primary focus:ring-2 cursor-pointer flex-shrink-0"
        >
        ...
        <button
            ...
            class="ml-2.5 p-1 text-ink-tertiary hover:text-warning transition-colors rounded star-btn flex-shrink-0 cursor-pointer"
            title="Toggle star"
        >
            @if($thread->is_flagged)
                <svg class="w-4 h-4 fill-current text-warning drop-shadow-xs" viewBox="0 0 20 20">
        ...
        {{-- Sender Avatar --}}
        <div class="ml-2 w-7 h-7 rounded-lg avatar-gradient-{{ $avatarIdx }} flex items-center justify-center text-xs font-bold shadow-2xs flex-shrink-0 hidden sm:flex">
            {{ $avatarChar }}
        </div>
        ...
        <a
            ...
            wire:navigate
            class="flex items-center min-w-0 flex-1 cursor-pointer ml-3"
        >
            {{-- Sender + Unread --}}
            <div class="flex items-center min-w-0 flex-1 pr-2">
                <span class="truncate block text-sm {{ !$thread->is_seen ? 'font-bold text-ink' : 'font-medium text-ink-secondary' }}">
                    {{ $thread->from_display ?? 'Unknown' }}
                </span>
                @if(!$thread->is_seen)
                    @php $unreadCount = $thread->unread_count ?? 0; @endphp
                    @if($unreadCount > 1)
                        <span class="ml-1.5 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold text-white bg-primary rounded-full flex-shrink-0 shadow-2xs">
                            {{ $unreadCount }}
                        </span>
                    @else
                        <span class="ml-1.5 w-2 h-2 bg-primary rounded-full flex-shrink-0 shadow-2xs"></span>
                    @endif
                @endif
            </div>
            ...
            <div class="flex items-center min-w-0 mr-4 flex-1">
                <span class="truncate block pr-2 text-xs sm:text-sm {{ !$thread->is_seen ? 'font-medium text-ink' : 'text-ink-secondary' }}">
                ...
                @foreach($displayLabels as $label)
                    <span class="label-chip inline-flex items-center px-1.5 py-0.5 text-[10px] font-semibold rounded text-white flex-shrink-0 ml-1 shadow-2xs"
            ...
            {{-- Date --}}
            <span class="text-xs {{ !$thread->is_seen ? 'font-semibold text-primary' : 'text-ink-tertiary' }} whitespace-nowrap ml-2 font-mono text-[11px]">{{ $thread->formatted_date ?? '' }}</span>
        </a>

        {{-- Attachment --}}
        @if($thread->has_attachments)
            <span class="p-1 text-ink-tertiary ml-2 flex-shrink-0" title="Has attachments">
        ...
        {{-- Expand chevron --}}
        <div class="ml-2 flex-shrink-0 p-1.5 cursor-pointer hover:bg-hover rounded-lg transition-colors" @click.stop="toggleExpand()">
            <svg
                class="w-4 h-4 text-ink-tertiary hover:text-ink transition-transform duration-150"
