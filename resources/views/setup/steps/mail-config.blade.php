<div class="p-8 sm:p-10">
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-900">Mail Server Configuration</h2>
    ...
    {{-- Auto-detection --}}
    <div class="mb-6">
        <label for="email_domain" class="block text-sm font-medium text-gray-700">
            Your email address
            <span class="font-normal text-gray-500">(optional — for auto-detecting server settings)</span>
        </label>
        <input type="email"
               id="email_domain"
               wire:model.blur="emailDomain"
               class="mt-1 block w-full max-w-sm rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border"
               placeholder="admin@example.com"
               autocomplete="email">
        <p class="mt-1 text-xs text-gray-500">Enter your full email address to auto-fill IMAP/SMTP settings for known providers.</p>

        @if ($detectedProvider)
            <div class="mt-2 flex items-center gap-2 text-sm text-green-800">
                <svg class="w-4 h-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
    ...
    <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="bg-gray-50 rounded-lg border border-gray-200 p-5">
            <h3 class="text-sm font-semibold text-gray-900 mb-4">
                <svg class="inline w-4 h-4 mr-1 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text"
                           ...
                           autocomplete="off">
                    @error('imapHost') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('imapUsername') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('imapPassword') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @if ($imapTestResult)
                        <div class="mt-2">
                            ...
                            @else
                                <p class="text-sm text-red-700">✗ {{ $imapTestResult['error'] }}</p>
                                @if (isset($imapTestResult['technical']))
                                    <div x-data="{ open: false }">
                                        <button type="button" @click="open = !open" class="text-xs text-blue-600 hover:underline mt-1">Technical details</button>
                                        <pre x-show="open" x-transition class="mt-1 p-2 bg-gray-100 rounded text-xs font-mono text-gray-700 overflow-auto max-h-24">{{ json_encode($imapTestResult['technical'], JSON_PRETTY_PRINT) }}</pre>
                                    </div>
                                @endif
                            @endif
                        </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text"
                           ...
                           autocomplete="off">
                    @error('smtpHost') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border"
                           autocomplete="username">
                    @error('smtpUsername') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="password"
                           ...
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border"
                           autocomplete="current-password">
                    @error('smtpPassword') <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p> @enderror
                </div>
                    ...
                    @if ($smtpTestResult)
                        <div class="mt-2">
                            @if ($smtpTestResult['success'])
                                <p class="text-sm text-green-700 font-medium">✓ Connected successfully</p>
                                <p class="text-xs text-gray-500 mt-0.5">No email was sent. Connection test only.</p>
                            @else
                                <p class="text-sm text-red-700">✗ {{ $smtpTestResult['error'] }}</p>
                                @if (isset($smtpTestResult['technical']))
                                    <div x-data="{ open: false }">
                                        <button type="button" @click="open = !open" class="text-xs text-blue-600 hover:underline mt-1">Technical details</button>
                                        <pre x-show="open" x-transition class="mt-1 p-2 bg-gray-100 rounded text-xs font-mono text-gray-700 overflow-auto max-h-24">{{ json_encode($smtpTestResult['technical'], JSON_PRETTY_PRINT) }}</pre>
                                    </div>
                                @endif
                            @endif
                        </div>
