...
>
    ...
    <div class="message-row flex items-center hover:bg-hover transition-colors duration-75 group">
        ...
        <div class="flex items-center gap-1 shrink-0">
            ...
            <button
                ...
                class="p-1 rounded star-btn shrink-0 cursor-pointer {{ $isStarred ? 'text-amber-500' : 'text-ink-tertiary/50 hover:text-amber-400' }}"
                title="Toggle star"
            >
                @if($isStarred)
                    <svg class="message-row-icon fill-amber-500 text-amber-500" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                @else
                    <svg class="message-row-icon fill-none stroke-currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    <div
        ...
        class="border-l-2 border-border-strong bg-surface-sunken/20 pl-3 py-0.5"
    >
        @if(!empty($thread->children) && count($thread->children) > 0)
            ...
            @if(count($thread->children) > 10)
                <div class="px-4 py-2 text-center">
                    <button class="text-xs text-primary hover:underline font-medium transition-colors cursor-pointer">
