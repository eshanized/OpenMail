            ...
            <div class="mb-6 p-4 bg-primary-subtle/50 border border-primary/20 rounded-md">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        <button wire:click="continueFromSession" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-hover text-sm">
                            Continue
                        </button>
                        <button wire:click="resetWizard" class="px-4 py-2 bg-surface-sunken text-ink-secondary rounded-md hover:bg-hover text-sm">
                            Start Over
                        </button>
                    </div>
                </div>
                ...
                @for ($i = 1; $i <= 8; $i++)
                    <li class="flex items-center">
                        <div class="flex items-center justify-center w-10 h-10 rounded-full border-2
                            @if ($i < 1) bg-primary border-primary text-white
                            @elseif ($i === 1) bg-primary border-primary text-white
                            @else bg-white border-border text-ink-tertiary
                            @endif
                            text-sm font-medium">
                            {{ $i }}
                        </div>
                        @if ($i < 8)
                            <div class="w-full sm:w-24 h-0.5
                                @if ($i < 1) bg-primary @else bg-surface-sunken @endif
                                ml-2 mr-2"></div>
                        @endif
                    </li>
                @endfor
            </ol>
            <div class="mt-4 text-center text-sm text-ink-tertiary">
